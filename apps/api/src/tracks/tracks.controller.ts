...
export class TracksController {
  ...
  @ApiOperation({
    ...
    description: 'Server-Sent Events endpoint streaming real-time extraction, platform matching, and timing progress.',
  })
  @ApiQuery({
    name: 'platform',
    required: false,
    description: 'Platform identifier (spotify, apple, deezer, netease, qq, isrc)',
    example: 'spotify',
  })
  @ApiQuery({
    name: 'id',
  ...
  @ApiResponse({
    status: 200,
    description: 'Server-Sent Events stream emitting lifecycle events: init, cache_hit, cache_miss, resolving, platform_matched, lyrics_searching, lyrics_found, saving, done, error',
    content: {
      'text/event-stream': {
        schema: {
          type: 'object',
  ...
  @ApiBadRequestResponse({ type: ErrorResponseDto, description: 'Missing required query parameters' })
  @Sse('lyrics/stream')
  streamLyrics(
  ...
  @ApiQuery({ name: 'platform', required: false, description: 'Platform identifier (spotify, apple, deezer, netease, qq, isrc)', example: 'spotify' })
  @ApiQuery({ name: 'id', required: false, description: 'Platform-specific track ID', example: '4cOdK2wGLETKBW3PvgPWqT' })
  @ApiQuery({ name: 'url', required: false, description: 'Direct song/track URL', example: 'https://open.spotify.com/track/4cOdK2wGLETKBW3PvgPWqT' })
  @ApiQuery({ name: 'format', required: false, enum: SUPPORTED_FORMATS, description: 'Output lyric format (default: json)', example: 'json' })
  @ApiQuery({ name: 'forceRefresh', required: false, type: Boolean, description: 'Bypass cached results and force live re-fetching', example: false })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Looks up a track in the database cache (~2ms) or fetches, resolves cross-platform links, and caches metadata in PostgreSQL.',
  })
  @ApiQuery({
    name: 'platform',
    required: false,
  ...
  @ApiQuery({
    name: 'url',
    required: false,
    description: 'Direct song/track URL from Spotify, Apple Music, Deezer, NetEase, or QQ Music',
    example: 'https://open.spotify.com/track/4cOdK2wGLETKBW3PvgPWqT',
  })
  @ApiOkResponse({
    type: SanitizedTrackDto,
    description: 'Sanitized track metadata with cross-platform IDs and lyric availability flag',
  })
  @ApiBadRequestResponse({
  ...
  @ApiOperation({
    ...
    description: 'Alternative JSON body endpoint to look up or synchronize track metadata.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Search indexed tracks in PostgreSQL database by title, artist, or album keyword.',
  })
  @ApiQuery({
    name: 'q',
    required: true,
  ...
  @ApiOperation({
    ...
    description: 'Retrieves sanitized track metadata by its internal PostgreSQL UUID.',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiOperation({
    ...
    description: 'Retrieves synchronized lyrics formatted into JSON, TTML, LRC, YRC, QRC, ASS, or other supported formats for an existing database track ID.',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiResponse({
    ...
    content: {
      'application/json': {
        schema: {
          type: 'array',
          description: 'Array of lines, each containing syllable/word token tuples [vocalType, startMs, lengthMs, text]',
          items: {
            type: 'array',
            items: {
              type: 'array',
              description: 'Word token: [vocalType (1=Lead, 2=BG, 3=Duet Lead, 4=Duet BG), startMs, lengthMs, text]',
            },
          },
          example: [
            [
  ...
  @ApiOperation({
    ...
    description: 'Fetches or synchronizes lyrics on-the-fly from upstream sources and converts them to the requested format.',
  })
  @ApiQuery({
    name: 'platform',
    required: false,
  ...
  @ApiBadRequestResponse({ type: ErrorResponseDto, description: 'Missing url or platform + id' })
  @ApiNotFoundResponse({ type: ErrorResponseDto, description: 'Lyrics not available for this track' })
  @Get('lyrics')
  async getLyrics(
    @Query('platform') platform?: string,
    @Query('id') id?: string,
