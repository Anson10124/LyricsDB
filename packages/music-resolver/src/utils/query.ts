...
export function normalizeSongTitle(rawTitle: string): TitleNormalizationResult {
  ...
  const isRemix = /\b(remix|vip|club\s+mix|extended\s+mix|dub\s+mix)\b/i.test(title);
  const isInstrumental = /\b(instrumental|karaoke\s+version|backing\s+track)\b/i.test(title);
  const isKaraoke = /\b(karaoke|sing-along|in\s+the\s+style\s+of)\b/i.test(title);
  const isCover = /\b(tribute\s+band|cover\s+version|tribute\s+to|originally\s+performed\s+by)\b/i.test(title);
  const isTribute = /\b(tribute\s+to|tribute\s+band|tribute\s+version)\b/i.test(title);
...
export function splitArtists(artistStr?: string): string[] {
  ...
  return cleaned
    .split(/(?:\s*,\s*|\s*;\s*|\s*&\s*|\s*\/\s*|\s+feat\.?\s+|\s+ft\.?\s+|\s+featuring\s+|\s+with\s+|\s+x\s+|\s+X\s+|\s+and\s+|、|，)/i)
    .map((a) => a.trim())
    .filter((a) => a.length > 0);
}
