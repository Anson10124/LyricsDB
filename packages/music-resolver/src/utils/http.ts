...
export class HttpClient {
  ...
  static async post<T>(url: string, payload: unknown, options: HttpOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      try {
        ...
        if (options?.payload !== undefined && options?.payload !== null) {
          if (options.payload instanceof URLSearchParams || typeof options.payload === 'string') {
