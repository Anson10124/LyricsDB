...
export class AppleMusicParser extends BaseMusicParser {
  ...
  parse(url: string): { id: string; type?: MetadataType; storefront?: string } {
    ...
    const trackParam =
      parsedUrl?.searchParams.get("i") || parsedUrl?.searchParams.get("track");
  ...
  async fetchMetadata(
    ...
    const country =
      options?.preferredCountry || storefront || this.defaultCountry;
    ...
    try {
      ...
      const detectedType: MetadataType =
        APPLE_MUSIC_TYPE_MAP[ogType] || itemType;
