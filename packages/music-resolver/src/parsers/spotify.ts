...
export const SPOTIFY_LINK_MOBILE_REGEX =
  /^https?:\/\/(?:spotify\.link|spotify\.app\.link|spoti\.fi)\/(\w+)/i;
...
export class SpotifyParser extends BaseMusicParser {
  ...
  parse(url: string): { id: string; type?: MetadataType } {
    ...
    if (desktopMatch) {
      return {
        id: desktopMatch[3] || "",
        type: this.mapRawType(desktopMatch[2]),
      };
    }
  ...
  async fetchMetadata(
    ...
    try {
      ...
      if (nextDataText) {
        ...
        const entity: SpotifyEmbedEntity | undefined =
