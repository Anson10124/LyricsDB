...
const KEY1 = new Uint8Array([
  33, 64, 35, 41, 40, 78, 72, 76, 105, 117, 121, 42, 36, 37, 94, 38,
]); // "!@#)(NHLiuy*$%^&"
const KEY2 = new Uint8Array([
  49, 50, 51, 90, 88, 67, 33, 64, 35, 41, 40, 42, 36, 37, 94, 38,
]); // "123ZXC!@#)(*$%^&"
const KEY3 = new Uint8Array([
  33, 64, 35, 41, 40, 42, 36, 37, 94, 38, 97, 98, 99, 68, 69, 70,
]); // "!@#)(*$%^&abcDEF"
...
function f(state: number, key: Uint8Array): number {
  ...
  return (
    (bitnumintl(sOut, 15, 0) |
      bitnumintl(sOut, 6, 1) |
      bitnumintl(sOut, 19, 2) |
      bitnumintl(sOut, 20, 3) |
      bitnumintl(sOut, 28, 4) |
      bitnumintl(sOut, 11, 5) |
      bitnumintl(sOut, 27, 6) |
      bitnumintl(sOut, 16, 7) |
      bitnumintl(sOut, 0, 8) |
      bitnumintl(sOut, 14, 9) |
      bitnumintl(sOut, 22, 10) |
      bitnumintl(sOut, 25, 11) |
      bitnumintl(sOut, 4, 12) |
      bitnumintl(sOut, 17, 13) |
      bitnumintl(sOut, 30, 14) |
      bitnumintl(sOut, 9, 15) |
      bitnumintl(sOut, 1, 16) |
      bitnumintl(sOut, 7, 17) |
      bitnumintl(sOut, 23, 18) |
      bitnumintl(sOut, 13, 19) |
      bitnumintl(sOut, 31, 20) |
      bitnumintl(sOut, 26, 21) |
      bitnumintl(sOut, 2, 22) |
      bitnumintl(sOut, 8, 23) |
      bitnumintl(sOut, 18, 24) |
      bitnumintl(sOut, 12, 25) |
      bitnumintl(sOut, 29, 26) |
      bitnumintl(sOut, 5, 27) |
      bitnumintl(sOut, 21, 28) |
      bitnumintl(sOut, 10, 29) |
      bitnumintl(sOut, 3, 30) |
      bitnumintl(sOut, 24, 31)) >>>
    0
  );
}
...
function desKeySetup(key: Uint8Array, encrypt: boolean): Uint8Array[] {
  ...
  const schedule: Uint8Array[] = Array.from(
    { length: 16 },
    () => new Uint8Array(6),
  );
  ...
  for (let i = 0; i < 16; i++) {
    ...
    for (let j = 0; j < 24; j++) {
      roundKey[Math.floor(j / 8)]! |= bitnumintr(
        C,
        KEY_COMPRESSION[j]!,
        7 - (j % 8),
      );
