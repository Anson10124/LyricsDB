...
function applyCasing(match: string, replacement: string): string {
  ...
  const isAllUpper = letters.length > 1 ? letters === letters.toUpperCase() : false;
  const isFirstUpper = match[0] === match[0]?.toUpperCase() && match[0] !== match[0]?.toLowerCase();
...
export function fixExplicitLyrics(payload: SyncedLyricsPayload): SyncedLyricsPayload {
